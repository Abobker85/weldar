...
use App\Models\SawRtReportWeldar;
use App\Models\SawCertificate;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Storage;
use Illuminate\Support\Facades\Log;

class SawRtReportController extends Controller
{
    /**
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $validated = $request->validate([
                ...
                'certificate_id' => 'required|exists:saw_certificates,id',
                'attachment' => 'required|file|mimes:pdf,jpg,jpeg,png|max:10240', // 10MB max
            ]);
            
            // Log request details for debugging
            Log::info('SAW RT Report Upload Request', [
            ...
            // Store the file
            $path = $request->file('attachment')->store('rt-reports/saw', 'public');
            
            // Create the RT report record
            $report = SawRtReportWeldar::create([
                'welder_id' => $request->welder_id,
                'certificate_id' => $request->certificate_id,
                'attachment' => $path,
            ]);
            ...
            Log::info('SAW RT Report created', ['report_id' => $report->id, 'path' => $path]);
            ...
            if ($isAjax) {
                ...
                return redirect()->route('saw-certificates.show', $request->certificate_id)
                    ->with('success', 'RT Report uploaded successfully.');
            }
        } catch (\Exception $e) {
            // Log the error
            Log::error('SAW RT Report upload error: ' . $e->getMessage(), [
                'file' => $e->getFile(),
                'line' => $e->getLine(),
