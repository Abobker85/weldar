...
use App\Models\FcawRtReportWeldar;
use App\Models\FcawCertificate;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Storage;
use Illuminate\Support\Facades\Log;

class FcawRtReportController extends Controller
{
    /**
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $validated = $request->validate([
                ...
                'certificate_id' => 'required|exists:fcaw_certificates,id',
                'attachment' => 'required|file|mimes:pdf,jpg,jpeg,png|max:10240', // 10MB max
            ]);
            
            // Log request details for debugging
            Log::info('FCAW RT Report Upload Request', [
            ...
            // Store the file
            $path = $request->file('attachment')->store('rt-reports/fcaw', 'public');
            
            // Create the RT report record
            $report = FcawRtReportWeldar::create([
                'welder_id' => $request->welder_id,
                'certificate_id' => $request->certificate_id,
                'attachment' => $path,
            ]);
            ...
            Log::info('FCAW RT Report created', ['report_id' => $report->id, 'path' => $path]);
            ...
            if ($isAjax) {
                ...
                return redirect()->route('fcaw-certificates.show', $request->certificate_id)
                    ->with('success', 'RT Report uploaded successfully.');
            }
        } catch (\Exception $e) {
            // Log the error
            Log::error('FCAW RT Report upload error: ' . $e->getMessage(), [
                'file' => $e->getFile(),
                'line' => $e->getLine(),
