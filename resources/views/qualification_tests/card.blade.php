...
<body>
    ...
    {{-- <button class="download-button" onclick="downloadCard()">Download HTML</button> --}}
    ...
    <div class="card-container" id="welderCard">
        <div class="card">
            ...
            <div class="card-main-title">
                <h1>WELDER QUALIFICATION CARD</h1>
            </div>
            ...
            <div class="card-content-area">
                <div class="info-left-column">
                    <div class="info-row">
                        ...
                        <div class="value">{{ App\Models\AppSetting::getValue('company_name', 'COMPANY NAME') }}</div>
                    </div>
                    <div class="info-row">
                        <div class="label">Welder Name:</div>
                        <div class="value">{{ $welder->name ?? 'N/A' }}</div>
                    </div>
                    <div class="info-row">
                        <div class="label">ID/Iqama No:</div>
                        <div class="value">{{ $welder->iqama_no ?? 'N/A' }}</div>
                    </div>
                    <div class="info-row">
                        <div class="label">Welder No:</div>
                        <div class="value">{{ $welder->welder_no ?? 'N/A' }}</div>
                    </div>
                    <div class="info-row">
                        <div class="label">Card No:</div>
                        <div class="value">{{ $qualification->cert_no ?? 'N/A' }}</div>
                    </div>
                </div>               
                
                <div class="photo-right-column">
                    @if(isset($welder->photo_path))
                        <img src="{{ asset('storage/' . $welder->photo_path) }}" alt="Welder Photo" style="max-width: 100%; max-height: 100%;">
                    @else
                        PHOTO
                    @endif
                </div>
            ...
            <div class="certification-statement-section">
                <p>This is to certify that the above named welder has been qualified according to {{ $qualification->qualification_code ?? 'AWS D1.1/API 1104' }} standards</p>
            </div>
            ...
            <div class="authorization-section">
                <div class="auth-details">
                    ...
                    <p class="auth-name">{{ Auth::user()->name ?? 'Inspector Name' }}</p>
                ...
                <div class="auth-date-container">
                    <p class="auth-date">Date: {{ now()->format('d-m-Y') }}</p>
                    <p class="auth-date">Expiry: {{ $qualification->test_date ? \Carbon\Carbon::parse($qualification->test_date)->addMonths(6)->format('d-m-Y') : 'N/A' }}</p>
                </div>
            </div>
              
            <!-- QR Code Section - Back at the bottom with negative margin -->
            <div class="barcode-section">
                <img src="{{ $qrCodeUrl }}" alt="QR Code" style="width:45px; height:45px;"> <!-- Reduced from 65px to 45px -->
                <div class="barcode-text">{{ $qualification->cert_no ?? 'N/A' }}</div>
    ...
    <script>
        // No barcode generation needed, using QR code image
        function downloadCard() {
            ...
            a.href = url;
            a.download = 'welder-qualification-card.html';
